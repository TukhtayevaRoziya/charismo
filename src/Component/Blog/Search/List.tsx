...
import { SearchVideoMap } from './Maps/SearchVideoMap'
import { AllType, ListType } from '../../../types'

import moduleName from './Search.module.css'
import { SearchAllMap } from './Maps/SearchAllMap';
import { SearchPhotoMap } from './Maps/SearchPhotoMap';
import { SearchTextMap } from './Maps/SearchTextMap';
...
class List extends React.Component<ListProps, ListState> {
    ...
    render() {
        ...
        let map = this.props.all.map((a: any) => <><p>{a.category}</p><h2>{a.country}</h2><h1>{a.author}</h1><h3>{a.description}</h3><img src={a.image} /><h4>{a.language}</h4><h5>{a.published_at}</h5><a href={a.source}>{a.source}</a><h6>title:{a.title}</h6><a href={a.url}>sdg</a></>)
        return (
            <div className={moduleName.box}>
                <input type="text" className={moduleName.inp} placeholder="Поиск..." onChange={this.handleChange} />
                <div className={moduleName.blocks_body}>
                    <Switch>
                        ...
                        <Route path={'/blog'} render={() => <SearchAllMap AllFiltered={this.state.AllFiltered} />} />
                    </Switch>
                    {map}
